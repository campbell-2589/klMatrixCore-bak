...
void testAddVSL(ofstream &_tex,__int64_t &n);
void testAdd(ofstream &_tex,__int64_t &n);
void testFreeMemoryChecker();
void testMKLMemoryManager(ofstream &_tex,__int64_t &n);
double vslBenchmark(__int64_t &n);
...
void klPerformanceTesting()
{
	...
	//for (__int64_t n = 1073741824; n<17179869184;n+=1073741824)
	...
	//i=0;
	//for (__int64_t n = 1024; n<=numIters*1024;n+=1024, i++)
	//{
	//	klutw.setDimension(n);
	//	klutw.runTest(testAddVSL);
	//	double vslDt = klutw.GetLastRuntime();
	...
	i=0;
	for (__int64_t n = 1; n<=numIters;n+=1, i++)
	{
		klutw.setDimension(n);
		{
			klMemMgr*  mgr = NULL;
...
void testFreeMemoryChecker()//(ofstream &_tex,unsigned int &n)
{
	...
	__int64_t maxAllocation=0;
	bool bigtest =klCheckFreeMemory(195290615808LL*195290615808LL);
	for(__int64_t n=4294967296;n<195290615808*512;n+=4294967296)
	{
		cout<<n<<endl;
...
void testMKLMemoryManager(ofstream &_tex,__int64_t &n)
{
	...
	for(__int64_t i=1048576;i<1048576 *numAllocs;i+=1048576)
	{
		...
		klVector<__int64_t> kli64(i);kli64=4294967296LL;
