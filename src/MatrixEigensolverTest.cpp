...
{	
	...
	_tex<<" Testing Operator Norm"<<endl<<"Number of Dimensions: "<<n;
...

	{
		...
		klVector<double> ans = SYEVX();

		klVector<double> spectrum = (ans);

		cout<<spectrum;

		klMatrix<double> E = SYEVX.Eigenvectors();
...

	{
		...
		klMatrix<double> prod = U * S * V;
		LatexPrintMatrix( prod,"U S V",_tex);
	}
